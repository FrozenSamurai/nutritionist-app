...
const EachRow = ({
  ...
  const handleChange = (select = selected, vol) => {
    if (vol === null || select === null) {
      ...
      let pro = ((vals["protein"] * vol) / vals["amount"]).toFixed(2);
      setProteinval(pro);
      let carb = ((vals["carbs"] * vol) / vals["amount"]).toFixed(2);
      setCarbval(carb);
      let fats = ((vals["fats"] * vol) / vals["amount"]).toFixed(2);
      setFatsval(fats);
      let energy = ((vals["energy"] * vol) / vals["amount"]).toFixed(2);
      setCalval(energy);
      let calcium = ((vals["calcium"] * vol) / vals["amount"]).toFixed(2);
      setCalcival(calcium);
      let fibre = ((vals["fibre"] * vol) / vals["amount"]).toFixed(2);
      setFibval(fibre);
      let iron = ((vals["iron"] * vol) / vals["amount"]).toFixed(2);
      setIronVal(iron);
      let alldata = {
        ...allEnteredData,
        [mealName + idnum]: {
