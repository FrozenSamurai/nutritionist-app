...
const EditMeal = ({
  ...
  let [rownum, setRownum] = useState(Object.keys(mealData).length);
  ...
  let [mealName, setMealName] = useState(mealname);
  ...
  return (
    <div className="w-4/5 mx-auto flex flex-col space-y-2 justify-center items-center">
      ...
      <>
        <TitleRow />
        <hr className="w-full" />
        {[...Array(rownum)].map((key, i) => {
          return (
            <EditRow
              key={i}
              row={{ ...mealData[Object.keys(mealData)[i]] }}
              idnum={i + 1}
